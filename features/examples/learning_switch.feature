Feature: "Learning Switch" sample application
  ...
  Background:
    Given the current example directory is "learning_switch"
    And a file named "learning_switch.conf" with:
      """
  ...
  Scenario: Run the C example
    ...
    And I run `trema run ./learning_switch -c learning_switch.conf -d`
  ...
  Scenario: Run the Ruby example
    Given I run `trema run learning-switch.rb -c learning_switch.conf -d`
