Feature: "Switch Info" sample application
  ...
  Background:
    Given the current example directory is "switch_info"
  ...
  Scenario: Run the C example
    Given a file named "switch_info.conf" with:
      ...
      """
    And I compile "switch_info.c" into "switch_info"
    And I run `trema run ./switch_info -c switch_info.conf -d`
    And *** sleep 2 ***
  ...
  Scenario: Run "Switch Info" Ruby example
    Given a file named "switch_info.conf" with:
      ...
      vswitch { datapath_id 0xabc }
      """
    And I run `trema run switch-info.rb -c switch_info.conf -d`
    And *** sleep 2 ***
