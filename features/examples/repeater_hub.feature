Feature: "Repeater Hub" sample application
  ...
  Background:
    Given the current example directory is "repeater_hub"
    And a file named "repeater_hub.conf" with:
      """
  ...
  Scenario: Run "Repeater Hub" C example
    Given I compile "repeater_hub.c" into "repeater_hub"
    And I run `trema run ./repeater_hub -c repeater_hub.conf -d`
  ...
  Scenario: Run "Repeater Hub" Ruby example
    Given I run `trema run repeater-hub.rb -c repeater_hub.conf -d`
